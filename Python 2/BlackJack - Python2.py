'''
Reworked some of the code for compatibilty with Python 2
Tested on Python 2.7.13

N Waterman v1 04-19-17
'''
...
def black_jack():
    ...
    dealer = Player()
    players = []
    player_count = int(raw_input("Enter the number of players: "))

    for x in range(0, player_count):
        name = raw_input("Enter the name of Player " + str(x+1) + ": ")
        name = Player(name)
        players.append(name)
...
def play_hand(players, player_count, dealer):
    ...
    for x in range(0, len(players)):
        ...
        players[x].make_hand()
        opening = int(raw_input("How much is opening bet? $"))
    ...
    for p in range(0, player_count):
        ...
        while stick == "no":
            ...
            if players[p].get_score() > 21:
                ...
                break
            elif raw_input(players[p].get_name() +
                           " would you like to stick? Y"
                           " or N \n").lower() != "y":
    ...
    elif raw_input("Would you like to play again? Y or N").upper() == "Y":
        play_hand(players, player_count, dealer)
    else:
        print("Thanks for playing")
